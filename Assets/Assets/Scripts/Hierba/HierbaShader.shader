Shader "Custom/ShaderHierba"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Función de ruido base 2D segura y ligera para GPU
            float RealWorldNoise(float2 p)
            {
                return frac(sin(dot(p, float2(127.1, 311.7))) * 43758.5453123);
            }

            // Interpolación bilineal corregida (sin bucles infinitos ni NaN)
            float ValueNoise2D(float2 p)
            {
                float2 i = floor(p);
                float2 f = frac(p);
                
                // Suavizado corregido usando la variable 'f'
            ...
            Varyings vert(Attributes input)
            {
                ...
                // --- CALCULO DE COLOR POR RUEDO (CORREGIDO) ---
                float noiseVal = ValueNoise2D(worldPos.xz * _NoiseScale); 
                float tintMap = (noiseVal * 2.0) - 1.0; // Rango -1.0 a 1.0
                ...
                // Máscara invertida (Raíz quieta, punta móvil)
                float mask = 1.0 - saturate(input.uv.y); 
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                // Aplicamos las tonalidades del Perlin Noise de forma segura
